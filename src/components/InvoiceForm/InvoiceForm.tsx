...
function InvoiceForm({ currency }: Props) {
  ...
  const handleSaveLocaleStorage = () => {
    try {
      ...
      localStorage.setItem(
        "invoiceLogoPreview",
        JSON.stringify(invoiceLogoPreview)
      );
  ...
  useEffect(() => {
    localStorage.getItem("senderName") &&
      setSenderName(JSON.parse(localStorage.getItem("senderName") || ""));
    localStorage.getItem("recipientName") &&
      setRecipientName(JSON.parse(localStorage.getItem("recipientName") || ""));
    localStorage.getItem("invoiceItems") &&
      setInvoiceItems(JSON.parse(localStorage.getItem("invoiceItems") || ""));
    localStorage.getItem("invoiceNote") &&
      setInvoiceNote(JSON.parse(localStorage.getItem("invoiceNote") || ""));
    localStorage.getItem("invoiceNumber") &&
      setInvoiceNumber(JSON.parse(localStorage.getItem("invoiceNumber") || ""));
    localStorage.getItem("invoiceLogoPreview") &&
      setInvoiceLogoPreview(
        JSON.parse(localStorage.getItem("invoiceLogoPreview") || "")
      );
    ...
    if (discountData !== null && discountData !== undefined) {
      ...
      localStorage.getItem("discountType") &&
        setDiscountType(JSON.parse(localStorage.getItem("discountType") || ""));
    }
    ...
    localStorage.getItem("showShipping") &&
      setShowShipping(JSON.parse(localStorage.getItem("showShipping") || ""));
  }, []);
  ...
  function calculateAndSetTotal(
    ...
    if (discountType === "percentage") {
      let calculatedTotal =
        subtotal * (1 - parseFloat(discount) / 100) +
        subtotal * (1 - parseFloat(discount) / 100) * (parseFloat(tax) / 100) +
        parseFloat(shipping);
      ...
      let calculatedTotal =
        subtotal -
        parseFloat(discount) +
        (subtotal - parseFloat(discount)) * (parseFloat(tax) / 100) +
        parseFloat(shipping);
      calculatedTotal = parseFloat(calculatedTotal.toFixed(2));
      setTotal(calculatedTotal);
    }
  ...
  const handleItemChange = (
    index: number,
    field: keyof { name: string; quantity: string; price: string },
    value: string
  ) => {
    // validate input
  ...
  // additional labels
  const showDueDateButton = (
    <button type="button" onClick={handleAddDueTo}>
      Add Due Date
    </button>
  );
  const dueDateToLabel = (
    <label>
      Due Date:
      {invoiceDueDate && (
        <input
          type="date"
          value={invoiceDueDate.toISOString().substr(0, 10)}
  ...
  const showDiscountButton = (
    <button type="button" onClick={toggleAddDiscount}>
      Add Discount
    </button>
  );
  const discountLabel = (
    <div className="add__label">
      <label className="add__label-label">
        Discount:
        <label>
          <input
            type="radio"
        ...
        <input
          ...
          onBlur={() => {
            if (isNaN(parseFloat(discount))) setDiscount("0");
          }}
        />
      </label>
      <label>
        <button
          className="add__button outline secondary"
          type="button"
  ...
  const showTaxButton = (
    <button type="button" onClick={toggleAddTax}>
      Add Tax
    </button>
  );
  const taxLabel = (
    <div className="add__label">
      <label className="add__label-label">
        ...
        <input
          ...
          onBlur={() => {
            if (isNaN(parseFloat(tax))) setTax("0");
          }}
        />
      </label>
      <label>
  ...
  const showShippingButton = (
    <button type="button" onClick={toggleAddShipping}>
      Add Shipping
    </button>
  );
  const shippingLabel = (
    <div className="add__label">
      <label className="add__label-label">
        ...
        <input
          ...
          onBlur={() => {
            if (isNaN(parseFloat(shipping))) setShipping("0");
          }}
        />
      </label>
      <label>
                ...
                <div className="invoice__info-subsection">
                  <div className="invoice__logo">
                    <label htmlFor="invoice__logo">
                      ...
                      <span className="invoice__logo-error">
                        {invoiceLogoError}
                      </span>
                    </label>
                    <div className="invoice__logo-preview">
                      {invoiceLogoPreview && (
                        <img
                          className="invoice__logo-preview-img"
                      ...
                      <label>
                        {invoiceLogoPreview ? (
                          <button
                            className="remove__button outline secondary"
                            type="button"
                ...
                <div className="invoice__info-subsection">
                  <label>
                    ...
                    <input
                      ...
                      onBlur={() => {
                        if (isNaN(parseFloat(invoiceNumber)))
                          setInvoiceNumber("1");
                      }}
                    />
                  </label>
            ...
            <section>
              ...
              {invoiceItems.map((item, index) => (
                <div className="invoice__items" key={index}>
                  ...
                  <label className="invoice__item-remove">
                    Remove
                    <button
                      className="invoice__item-button"
                      type="button"
        ...
        >
          {({ loading }) =>
            loading ? (
              ...
              <button onClick={handleSaveLocaleStorage} type="submit">
                Download invoice
              </button>
            )
          }
